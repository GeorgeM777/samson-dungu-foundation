        ...
        @forelse($stories as $story)
            <details style="background: #f8fafc; border-radius: 8px; padding: 15px; margin-bottom: 10px;">
                <summary style="cursor: pointer; display: flex; align-items: center; gap: 15px;">
                    @if($story->image)
                        <img src="{{ asset('storage/' . ltrim(str_replace('/storage/', '', $story->image), '/')) }}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 8px;">
                    @endif
                    ...
                    <div class="form-group">
                        ...
                        @if($story->image)
                            <div class="upload-preview-wrap active" style="display: inline-block;">
                                ...
                                <img src="{{ asset('storage/' . ltrim(str_replace('/storage/', '', $story->image), '/')) }}">
                            </div>
                        @endif
                    </div>
                    <div class="form-group">
